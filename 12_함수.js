...
/*
함수 내부에 정의된 함수(==내부 함수)
중첩 함수는 외부 함수 내에서만 호출 가능하다. => 중첩 함수는 헬퍼 함수(외부의 함수를 도움)의 역할을 많이 한다.

함수 선언문은 ES6 이후부터 if문이나 for문 등의 코드 블록 내에도 정의 가능하다.
하지만 호이스팅으로 인해 혼란이 발생할 수 있으므로 if문 for문 코드블록 내에 정의하는 것은 지양하자.
...
//4. 콜백 함수
/*
어떤 일을 반복하는 함수가 있다고 하자(ex. 5가 들어오면 0부터 4까지 5번 출력.)
만약 똑같이 5가 들어오지만, 이번에는 홀수일 때만 출력한다 치자.
그렇다면 분명 똑같은 반복 행동을 하지만 다른 일을 하지 못하기 때문에 각각 정의해야 한다.
=> 반복하는 일을 공통적으로 수행하지만, 실행하는 일의 내용이 다르기 때문에 새로운 함수를 만들어야 한다
==> 즉, 함수의 일부분만이 다르기 때문에 매번 함수를 새롭게 정의해야 한다.

이에 대한 해결책이 '콜백 함수'이다.
공통된 특성은 따로 정의해두고, 경우에 따라 변경되는 로직은 추상화하여 함수 외부에서 내부로 전달하는 것.
 */
//공통된 로직을 정의해둔다.(고차 함수)
function repeat(n, f){
    for(var i=0; i<n; i++){
        f(i);       //경우에 따라 변경되는 일을 추상화해둔 함수에 i를 전달
    }
}
//모두 출력하는 함수(콜백 함수)
...
repeat(5, logOdd);
/*
콜백 함수 : 함수의 매개변수를 통해 다른 함수의 내부로 전달되는 함수, 고차 함수에 의해 호출
고차 함수 : 매개 변수를 통해 함수의 외부에서 콜백 함수를 전달받은 함수, 필요에 따라 콜백 함수에 인수를 전달
-> 고차 함수는 콜백 함수를 자신의 일부분으로 합성한다!

고차 함수에 콜백 함수를 전달할 때 콜백 함수를 호출하는 것이 아니라, 함수 자체를 전달해야 한다.
repeat에서 함수 표현식을 적었다면, 한 번 실행될 때마다 함수 객체가 생성된다.
그래서 함수 외부에 콜백 함수를 정의한 후 함수 참조를 고차 함수에 전달하는 편이 효율적이다.

***콜백 함수는 함수형 프로그래밍 패러다임뿐 아니라 비동기 처리(이벤트 처리, Ajax통신, 타이머 함수)등에 활용되는 중요한 패턴***
콜백 함수는 비동기 처리 뿐 아니라 배열 고차 하뭇에서도 사용된다!!!
map, filter, reduce
 */
...
순수 함수는 오직 매개변수를 통해 함수 내부로 전달된 인수에게만 의존해 반환값을 만든다. 또한 외부 상태를 변경하지도 않는다.
비순수 함수는 외부 상태에 따라 반환하는 값이 달라진다. 또한 외부 상태를 변경하는 부수 효과(side effect)가 있다.

1. 외부 상태를 직접 참조하면 외부 상태에 의존하게 되어 반환값이 변할 수 있고, 외부 상태도 변경할 수 있음
2. 직접 참조하지 않더라도 매개변수를 통해 객체를 전달받으면 비순수 함수가 된다.

함수가 외부 상태를 변경하면 상태 변화를 추적하기 어려워진다. 따라서 순수함수를 지향하자.
 */

//함수형 프로그래밍!
/*
함수형 프로그래밍은 순수 함수와 보조 함수의 조합을 통해 외부 상태를 변경하는 부수 효과를 최소화해서 불변성을 지향하는 프로그래밍 패러다임
1. 조건문이나 반복문을 제거해서 복잡성을 해결 => 로직의 흐름을 이해하기 어렵게 해서 가독성을 해침
2. 변수 사용을 억제하거나 생명 주기를 최소화해서 상태 변경을 피해 오류를 최소화 => 누군가에 의해 변경되면 오류가 발생되므로
하는 것이 목적이다.

자바스크립트는 객체지향 프로그래밍뿐 아니라 함수형 프로그래밍을 적극적으로 활용하고 있다.
 */